...
async function getSectionData(slug: string, id: string) {
  try {
    ...
    const uiRecords = records.map((record) => ({
      ...
      fobValue: record.fob_value ? `KES ${Math.round(Number(record.fob_value)).toLocaleString()}` : 'N/A',
      dutyPaid: record.import_duty ? `KES ${Math.round(Number(record.import_duty)).toLocaleString()}` : 'N/A',

...
export default async function MacroIndicatorsPage({
  ...
  try {
    const [resultRows, totalRows, yearRows, indicatorRows] = await Promise.all([
      prisma.$queryRaw<Row[]>`
        SELECT id, indicator_code, indicator_name, country_iso2, country_iso3, country_name, year, value, unit, obs_status
        FROM macro_data.indicators
        ${whereClause}
      ...
      prisma.$queryRaw<Array<{ year: number | null }>>`
        SELECT year FROM macro_data.indicators GROUP BY year ORDER BY year DESC
      `,
    ...
    console.error("Macro indicators query failed", e);
    ...
    <div className="min-h-screen bg-[#F1F5F9] font-[Poppins] text-slate-700">
      <header className="bg-[#1e3a8a] pt-10 pb-16 text-white">
        <div className="container mx-auto px-6">
          ...
          <h1 className="text-3xl md:text-4xl font-semibold tracking-tight">Macro Indicators</h1>
          <p className="text-blue-200 mt-2 text-sm italic font-mono">Database: macro_data.indicators ({total.toLocaleString()} records)</p>
        ...
        <aside className="w-full xl:w-[300px] shrink-0">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 sticky top-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href="/research/macro/indicators" className="text-slate-300 hover:text-blue-600 transition"><RotateCcw className="h-3.5 w-3.5" /></Link>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <tbody className="divide-y border-t">
                  ...
                  {rows.map((r) => (
                    <tr key={r.id} className="hover:bg-blue-50/40 transition-colors">
                      ...
                      <td className="px-6 py-4"><div className="font-medium text-slate-700">{r.indicator_name}</div><div className="text-[10px] text-slate-400 uppercase">{r.indicator_code}</div></td>
                      <td className="px-6 py-4 text-right font-mono font-bold">{r.value == null ? "-" : Number(r.value).toFixed(2)}</td>
                      <td className="px-6 py-4 text-xs text-slate-600">{r.unit || "N/A"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="p-4 bg-slate-50 border-t flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <Link href={`/research/macro/indicators?${new URLSearchParams({ ...Object.fromEntries(baseParams), page: String(page - 1) }).toString()}`} className={`text-[11px] font-bold uppercase px-4 py-2 border rounded-xl bg-white transition hover:bg-slate-50 ${page <= 1 ? "opacity-20 pointer-events-none" : ""}`}>Back</Link>
                <Link href={`/research/macro/indicators?${new URLSearchParams({ ...Object.fromEntries(baseParams), page: String(page + 1) }).toString()}`} className={`text-[11px] font-bold uppercase px-4 py-2 bg-[#1e3a8a] text-white rounded-xl shadow-md transition hover:bg-blue-800 ${page >= totalPages ? "opacity-20 pointer-events-none" : ""}`}>Next</Link>
              </div>
            </div>
          </div>
        </main>

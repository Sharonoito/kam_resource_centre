...
export default async function ResearchPage() {
  ...
  const mergedDocs = [
    ...docs.map((d: any) => ({
      ...d,
      // For SharePoint docs, we still use the download/proxy logic
    ...
    ...kamContent.map((c: any) => ({
      ...
      // Fix: Extract year from published_date
      year: c.published_date ? new Date(c.published_date).getFullYear() : 2026,
      file_size_bytes: c.file_size_bytes || 0,
      // Internal viewer fallback path
          ...
          <div className="container mx-auto px-6 max-w-7xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {allDocs.map((doc: any, index: number) => {
                ...
                // Internal route for viewing
                const internalViewUrl = `/resources/${doc.id}`;
                    ...
                    <div key={`${doc.id}-${index}`} className={cardBase}>
                      ...
                      <div className="p-4 flex flex-col flex-1">
                        ...
                        <div className="mt-auto">
                          {/* FIXED: Link to internal viewer instead of external URL */}
                          <Link 
                  ...
                  <div key={`${doc.id}-${index}`} className={cardBase}>
                    <div className="relative h-32 overflow-hidden bg-slate-50 border-b border-slate-100 flex items-center justify-center">
                      ...
                      <div className="absolute inset-0 bg-[#0B1E3A]/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                         <Link href={internalViewUrl} className="px-3 py-1 bg-white text-[#193C8D] text-[9px] font-black rounded-full uppercase shadow-xl">Quick Read</Link>
                      </div>
                    </div>
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <Link href={internalViewUrl} className="block mb-4">
                      ...
                      <div className="mt-auto flex items-center justify-between pt-3 border-t border-slate-50">
                        <Link href={internalViewUrl} className="text-[10px] font-black text-[#193C8D] hover:text-[#E7B947] uppercase tracking-tighter transition-colors">
                          View Online
                        </Link>
      ...
      <footer className="py-12 bg-white border-t border-slate-100">
        <div className="container mx-auto px-6 text-center">
            <p className="text-slate-300 text-[10px] font-bold uppercase tracking-[0.3em] flex items-center justify-center gap-4">
              ...
              KAM Research Hub {new Date().getFullYear()}
              <span className="w-8 h-[1px] bg-slate-200"></span>
            </p>
        </div>
      </footer>

...
// import { 
//   Search, Filter, Globe, ArrowRight, Plus, ChevronDown,
//   BarChart3, TrendingUp, X, ArrowDownLeft, ArrowUpRight, Clock, Activity
// } from 'lucide-react'
...
//     if (isNewSearch) { setLoading(true); setHasMore(true); } 
...
//         setStatsData(stats); setTrendData(Array.isArray(trend) ? trend : []); setDbData(newData);
//       } else { setDbData(prev => [...prev, ...newData]); }
...
//     setPage(nextPage); fetchData(nextPage, false);
//   }

//   const getMonthName = (monthStr: string | null) => {
//     if (!monthStr) return '';
//     const m = monthStr.split('-')[1] || monthStr;
...
//                 <button 
//                   onClick={() => {
//                     setSelectedYear(''); setSelectedMonth(''); setSelectedCountry(''); 
//                     setSelectedFlow(''); setSelectedSubcategory('')
//                   }} 
...
//               {/* COMBINED REPORT CATEGORY & SUBCATEGORY */}
//               <div className="mb-8">
...
//                     className={`w-full p-5 bg-white border border-zinc-100 rounded-3xl text-left shadow-sm hover:shadow-md transition-all flex justify-between items-center group ${isCategoryOpen ? 'ring-2 ring-blue-100' : ''}`}
//                   >
//                     <div className="flex flex-col">
//                         <span className="text-[9px] font-black text-zinc-400 uppercase">ICMS DATA</span>
//                         <span className="text-xs font-black text-[#193C8D] uppercase tracking-tight">
//                         {selectedSubcategory ? selectedSubcategory.replace('_', ' ') : 'ALL REPORTS'}
//                         </span>
//                     </div>
//                     <ChevronDown className={`w-4 h-4 text-[#193C8D] transition-transform ${isCategoryOpen ? 'rotate-180' : ''}`} />
//                   </button>

//                   {isCategoryOpen && (
//                     <div className="absolute z-30 w-full mt-2 bg-white border border-zinc-100 rounded-[1.5rem] shadow-2xl py-3 overflow-hidden animate-in fade-in slide-in-from-top-2">
//                       <div className="px-5 py-2 mb-2 border-b border-zinc-50">
//                         <p className="text-[8px] font-black text-zinc-300 uppercase tracking-widest">Select Sub-Report</p>
//                       </div>
//                       {[
//                         { label: 'ALL REPORTS', value: '' },
//                         { label: 'TAX REPORTS', value: 'TAX_REPORTS' },
//                         { label: 'IMPORTS REPORTS', value: 'IMPORTS_REPORTS' },
//                         { label: 'EXPORTS REPORTS', value: 'EXPORTS_REPORTS' }
//                       ].map((opt) => (
//                         <button
//                           key={opt.label}
//                           onClick={() => {
...
//                           }}
//                           className={`w-full px-6 py-3 text-left text-[10px] font-black uppercase tracking-widest transition-colors ${selectedSubcategory === opt.value ? 'bg-blue-50 text-[#193C8D]' : 'text-zinc-500 hover:bg-zinc-50 hover:text-[#193C8D]'}`}
//                         >
//                           {opt.label}
//                         </button>
//                       ))}
...
//                     className="w-full p-5 bg-white border border-zinc-100 rounded-3xl text-left shadow-sm hover:shadow-md transition-all flex justify-between items-center"
//                   >
//                     <span className="text-xs font-black text-[#193C8D] uppercase tracking-tight truncate mr-2">
//                       {selectedCountry || 'GLOBAL (ALL)'}
//                     </span>
//                     <ChevronDown className={`w-4 h-4 text-[#193C8D] transition-transform ${isCountryOpen ? 'rotate-180' : ''}`} />
//                   </button>
//                   {isCountryOpen && (
//                     <div className="absolute z-30 w-full mt-2 bg-white border border-zinc-100 rounded-[1.5rem] shadow-2xl py-2 max-h-48 overflow-y-auto animate-in fade-in slide-in-from-top-2">
//                       <button onClick={() => {setSelectedCountry(''); setIsCountryOpen(false)}} className="w-full px-5 py-3 text-left text-[10px] font-black text-[#193C8D] uppercase hover:bg-zinc-50">GLOBAL (ALL)</button>
//                       {meta.countries?.map(c => (
//                         <button key={c} onClick={() => {setSelectedCountry(c); setIsCountryOpen(false)}} className="w-full px-5 py-3 text-left text-[10px] font-black text-[#193C8D] uppercase hover:bg-zinc-50">{c}</button>
//                       ))}
//                     </div>
//                   )}
//                 </div>
...
//                   >
//                     <span className="text-xs font-black text-[#193C8D] uppercase tracking-tight">
//                       {selectedMonth ? getMonthName(selectedMonth) : 'FULL YEAR'}
//                     </span>
//                     <ChevronDown className={`w-4 h-4 text-[#193C8D] transition-transform ${isMonthOpen ? 'rotate-180' : ''}`} />
//                   </button>
//                   {isMonthOpen && (
//                     <div className="absolute z-30 w-full mt-2 bg-white border border-zinc-100 rounded-[1.5rem] shadow-2xl py-2 max-h-48 overflow-y-auto animate-in fade-in slide-in-from-top-2">
//                       <button onClick={() => {setSelectedMonth(''); setIsMonthOpen(false)}} className="w-full px-5 py-3 text-left text-[10px] font-black text-[#193C8D] uppercase hover:bg-zinc-50">FULL YEAR</button>
//                       {['01','02','03','04','05','06','07','08','09','10','11','12'].map((m, i) => (
//                         <button key={m} onClick={() => {setSelectedMonth(m); setIsMonthOpen(false)}} className="w-full px-5 py-3 text-left text-[10px] font-black text-[#193C8D] uppercase hover:bg-zinc-50">
...
//                   </h2>
//                   <div className="text-zinc-400 text-sm font-bold uppercase tracking-widest flex items-center gap-2">
...
//                 <div className="text-3xl font-black text-[#193C8D]">
...
//                                 <div className="text-2xl font-black text-[#193C8D]">
//                                     <span className="text-sm text-zinc-400 mr-1 font-bold">KES</span>
//                                     {Number(item.fob_value).toLocaleString()}
//                                 </div>
//                             </div>
//                             <Link href={`/research/barometer/analytics/${item.id}`} className="flex items-center gap-3 px-8 py-4 bg-[#193C8D] text-white rounded-2xl hover:bg-[#E7B947] hover:text-[#193C8D] transition-all font-black uppercase text-[10px] tracking-widest shadow-lg shadow-blue-900/10">
...
//                     <button onClick={handleLoadMore} disabled={loadingMore} className="flex items-center gap-4 px-12 py-6 bg-white border-2 border-[#193C8D] text-[#193C8D] rounded-[2rem] font-black uppercase text-[11px] tracking-[0.2em] hover:bg-[#193C8D] hover:text-white transition-all shadow-xl disabled:opacity-50 group">
//                       {loadingMore ? <div className="w-5 h-5 border-t-2 border-[#E7B947] rounded-full animate-spin" /> : <Plus className="w-5 h-5 group-hover:rotate-90 transition-transform" />}
